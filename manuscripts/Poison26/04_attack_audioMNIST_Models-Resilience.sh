...
OUTPUT=$WORKINGDIR/audioMNIST_test_model1
mkdir -p $OUTPUT
cd $OUTPUT
...
#    echo "Preparing cohort: $COHORT_ID"
    ...
    echo "singularity exec -B $WORKINGDIR/models:/models $SIF bash -c \"time python $POISON --modelPath /models/$MODEL --epochs $EPOCH --particleNum $PARTICLE --maxRetries $RETRY --outputPath audioMNIST_test_$index --targetLabel $falseLabel --sourceIndex $index\"" >> $OUTPUT/attack_$COHORT_ID.slurm
    ((COHORT_INDEX++))
