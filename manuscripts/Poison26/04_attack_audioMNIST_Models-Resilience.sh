...
# Legacy image explicitly specified for audio scripts
...
# ==========================================
MODEL_NAME="audiomnist_basic_standard"
ARCH="basic"
...
while read line; do
    ...
    echo "singularity exec -B $WORKINGDIR/models:/models $SIF bash -c \"time python $POISON --modelPath /models/$MODEL_FILE --arch $ARCH --epochs $EPOCH --particleNum $PARTICLE --maxRetries $RETRY --outputPath audioMNIST_test_$index --targetLabel $falseLabel --sourceIndex $index\"" >> $OUTPUT/attack_$COHORT_ID.slurm
    ((COHORT_INDEX++))
