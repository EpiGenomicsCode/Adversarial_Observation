...
def train(model, train_loader, device, epochs=10, lr=0.001, adv=True, eps=0.05, alpha=0.01, pgd_steps=10, random_start=True, adv_lambda=1.0):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}", unit="batch"):
            ...
            if adv:
                model.eval()
                inputs_adv = pgd_attack(model, inputs, labels, eps=eps, alpha=alpha, steps=pgd_steps, random_start=random_start, clamp_min=-1.0, clamp_max=1.0)
                model.train()
            else:
                inputs_adv = None
...
def evaluate_model(model, test_loader, device, robust=False, eps=0.05, alpha=0.01, pgd_steps=10):
    ...
    for inputs, labels in test_loader:
        ...
        if robust:
            inputs_in = pgd_attack(model, inputs, labels, eps=eps, alpha=alpha, steps=pgd_steps, random_start=True, clamp_min=-1.0, clamp_max=1.0)
        else:
            inputs_in = inputs
...
def main():
    ...
    parser.add_argument("--output", type=str, default="audiomnist_dynamiclstm_pgd.pt")
